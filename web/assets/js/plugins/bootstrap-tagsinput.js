...
(function ($) {
  ...
  function TagsInput(element, options) {
    ...
    this.$input = $('<input type="text" class="form-control" placeholder="'
        + this.placeholderText + '"/>').appendTo(this.$container);
  ...
  TagsInput.prototype = {
    ...
    add: function (item, dontPushVal, options) {
      ...
      if (typeof item === "string" && this.$element[0].tagName === 'INPUT') {
        var delimiter = (self.options.delimiterRegex)
            ? self.options.delimiterRegex : self.options.delimiter;
      ...
      var $tag = $(
          '<span class="tag ' + htmlEncode(tagClass) + (itemTitle !== null
      ...
      if (self.objectItems) {
        if (typeof item === "object") {
          item = $.grep(self.itemsArray, function (other) {
            return self.options.itemValue(other) == self.options.itemValue(
                item);
          });
        } else {
          item = $.grep(self.itemsArray, function (other) {
    ...
    build: function (options) {
      ...
      if (self.options.typeahead) {
        ...
        self.$input.typeahead($.extend({}, typeahead, {
          ...
          matcher: function (text) {
            return (text.toLowerCase().indexOf(this.query.trim().toLowerCase())
                !== -1);
          },
      ...
      if (self.options.typeaheadjs) {
        ...
        $.fn.typeahead.apply(self.$input, typeaheadjs).on('typeahead:selected',
      ...
      self.$container.on('keypress', 'input', $.proxy(function (event) {
        ...
        if (self.options.freeInput && (keyCombinationInList(event,
