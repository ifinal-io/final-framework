package cn.com.likly.finalframework.data.repository;

import cn.com.likly.finalframework.data.domain.Criteria;
import cn.com.likly.finalframework.data.domain.Query;
import cn.com.likly.finalframework.data.domain.Update;
import cn.com.likly.finalframework.data.entity.IEntity;
import cn.com.likly.finalframework.data.mapping.Entity;

import java.io.Serializable;
import java.util.Arrays;
import java.util.Collection;
import java.util.List;
import java.util.stream.Collectors;

/**
 * @author likly
 * @version 1.0
 * @date 2018-10-22 21:00
 * @since 1.0
 */
public interface RepositoryTemplate<ID extends Serializable, T extends IEntity<ID>> {

    /*=========================================================== INSERT ===========================================================*/

    default int insert(Entity<T> holder, T... entities) {
        return insert(holder, Arrays.asList(entities));
    }

    int insert(Entity<T> holder, Collection<T> entities);

    default int insertUuid(Entity<T> holder, T... entities) {
        return insertUuid(holder, Arrays.asList(entities));
    }

    int insertUuid(Entity<T> holder, Collection<T> entities);

    default int insertMd5(Entity<T> holder, T... entities) {
        return insertMd5(holder, Arrays.asList(entities));
    }

    int insertMd5(Entity<T> holder, Collection<T> entities);

    default int insertOther(Entity<T> holder, T... entities) {
        return insertOther(holder, Arrays.asList(entities));
    }

    int insertOther(Entity<T> holder, Collection<T> entities);


    /*=========================================================== UPDATE ===========================================================*/

    default int update(Entity<T> holder, T entity) {
        return update(holder, entity, new Query().where(Criteria.where(holder.getIdProperty()).is(entity.getId())));
    }

    default int update(Entity<T> holder, T entity, Query query) {
        return update(holder, entity, null, query);
    }

    default int update(Entity<T> holder, Update update, ID... ids) {
        return update(holder, update, Arrays.asList(ids));
    }

    default int update(Entity<T> holder, Update update, Collection<ID> ids) {
        return update(holder, update, new Query().where(Criteria.where(holder.getRequiredIdProperty()).in(ids)));
    }

    default int update(Entity<T> holder, Update update, Query query) {
        return update(holder, null, update, query);
    }

    int update(Entity<T> holder, T entity, Update update, Query query);


    /*=========================================================== DELETE ===========================================================*/

    default int delete(Entity<T> holder, T... entities) {
        return delete(holder, Arrays.asList(entities));
    }

    default int delete(Entity<T> holder, List<T> entities) {
        return delete(holder, entities.stream().map(IEntity::getId).collect(Collectors.toList()));
    }

    default int delete(Entity<T> holder, ID... ids) {
        return delete(holder, Arrays.asList(ids));
    }

    default int delete(Entity<T> holder, Collection<ID> ids) {
        return delete(holder, new Query().where(Criteria.where(holder.getIdProperty()).in(ids)));
    }

    int delete(Entity<T> holder, Query query);


    /*=========================================================== SELECT ===========================================================*/
    default List<T> select(Entity<T> holder, ID... ids) {
        return select(holder, Arrays.asList(ids));
    }

    default List<T> select(Entity<T> holder, Collection<ID> ids) {
        Criteria in = Criteria.where(holder.getRequiredIdProperty()).in(ids);
        return select(holder, new Query().where((Criteria) in));
    }

    List<T> select(Entity<T> holder, Query query);

    default T selectOne(Entity<T> holder, ID id) {
        return selectOne(holder, new Query().where((Criteria) Criteria.where(holder.getRequiredIdProperty()).is(id)));
    }

    T selectOne(Entity<T> holder, Query query);

    default long selectCount(Entity<T> holder) {
        return selectCount(holder, null);
    }

    long selectCount(Entity<T> holder, Query query);


    default boolean isExists(Entity<T> holder, ID id) {
        return selectOne(holder, id) != null;
    }

    default boolean isExists(Entity<T> holder, Query query) {
        return selectCount(holder, query) > 0;
    }

}
