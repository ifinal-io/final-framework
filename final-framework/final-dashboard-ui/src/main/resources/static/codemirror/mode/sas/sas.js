  ...
  CodeMirror.defineMode("sas", function () {
    ...
    function tokenize(stream, state) {
      ...
      if (word && words.hasOwnProperty(word)) {
        ...
        if (state.inDataStep === true && words[word].state.indexOf("inDataStep")
            !== -1) {
          return words[word].style;
        }
        if (state.inProc === true && words[word].state.indexOf("inProc")
            !== -1) {
          return words[word].style;
        }
        if (state.inMacro === true && words[word].state.indexOf("inMacro")
            !== -1) {
          return words[word].style;
        }
