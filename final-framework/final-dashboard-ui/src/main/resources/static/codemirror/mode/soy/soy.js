...
(function (mod) {
  ...
  CodeMirror.defineMode("soy", function (config) {
    ...
    function ref(list, name, loose) {
      return contains(list, name) ? "variable-2" : (loose ? "variable"
          : "variable-2 error");
    }
    ...
    function expression(stream, state) {
      ...
      if (stream.match(/[[]/)) {
        ...
        state.context = new Context(state.context, "list-literal",
            state.variables);
    ...
    return {
      ...
      token: function (stream, state) {
            ...
            if (stream.match(/^\/?}/)) {
              ...
              if (state.tag == "/template" || state.tag == "/deltemplate") {
                ...
                state.indent -= config.indentUnit *
                    (selfClosed || indentingTags.indexOf(state.tag) == -1 ? 2
                        : 1);
              }
              state.soyState.pop();
              return "keyword";
              ...
              if (state.context && state.context.tag == tagName
                  && stream.current() == "kind" && (match = stream.match(
                      /^="([^"]+)/, false))) {
                var kind = match[1];
        ...
        if (stream.match(/^\{literal}/)) {
          ...
          var tag = tags[tagName];
          if (state.tag != "/switch") {
            state.indent += ((endTag || tag && tag.reduceIndent) && prevTag
            != "switch" ? 1 : 2) * config.indentUnit;
          }
