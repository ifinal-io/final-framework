...
(function (global, factory) {
    ...
    if (style || startStyle || endStyle || mustWrap || css || attributes) {
      ...
      if (attributes) {
        for (var attr in attributes) {
          if (attributes.hasOwnProperty(attr) && attr != "style" && attr
              != "class") {
  ...
  function buildTokenBadBidi(inner, order) {
    return function (builder, text, style, startStyle, endStyle, css,
      ...
      for (; ;) {
        ...
        if (part.to >= end) {
          return inner(builder, text, style, startStyle, endStyle, css,
              attributes)
        }
        ...
        if (text) {
          ...
          if (!collapsed) {
            ...
            builder.addToken(builder, tokenText,
                ...
                spanStartStyle,
                pos + tokenText.length == nextChange ? spanEndStyle : "", css,
                attributes);
          }
          if (end >= upto) {
    ...
    for (var i = 0; i < map.length; i += 3) {
      ...
      if (start != null) {
        ...
        if (bias == "right" && start == mEnd - mStart) {
          while (i < map.length - 3 && map[i + 3] == map[i + 4] && !map[i
          + 5].insertLeft) {
            node = map[(i += 3) + 2];
            collapse = "right";
          }
  ...
  function coordsChar(cm, x, y) {
    ...
    if (lineN > last) {
      return PosWithInfo(doc.first + doc.size - 1,
          getLine(doc, last).text.length, null, 1, 1)
    }
    if (x < 0) {
      x = 0;
    }
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    if (boxAround) {
      ...
      // base X position
      var coords = cursorCoords(cm, Pos(lineNo, ch, sticky), "line", lineObj,
          preparedMeasure);
      baseX = coords.left;
  ...
  function posFromMouse(cm, e, liberal, forRect) {
    ...
    if (forRect && coords.xRel > 0 && (line = getLine(cm.doc,
    ...
    for (var i = 0; i < display.view.length; i++) {
      ...
      if (ie && ie_version < 8) {
        ...
        if (!wrapping && cur.text.firstChild) {
          width = cur.text.firstChild.getBoundingClientRect().right - box.left
              - 1;
        }
      }
  ...
  function calculateScrollPos(cm, rect) {
    ...
    var screenleft = cm.curOp && cm.curOp.scrollLeft != null
        ? cm.curOp.scrollLeft : display.scroller.scrollLeft - gutterSpace;
    ...
    if (line.markedSpans) {
      for (var i = 0; i < line.markedSpans.length; ++i) {
        ...
        var preventCursorLeft = ("selectLeft" in m) ? !m.selectLeft
            : m.inclusiveLeft;
        var preventCursorRight = ("selectRight" in m) ? !m.selectRight
            : m.inclusiveRight;

        if ((sp.from == null || (preventCursorLeft ? sp.from <= pos.ch : sp.from
            < pos.ch)) &&
            (sp.to == null || (preventCursorRight ? sp.to >= pos.ch : sp.to
                > pos.ch))) {
          if (mayClear) {
        ...
        if (++read == n) {
          operation(cm, function () {
            ...
            setSelectionReplaceHistory(cm.doc,
                simpleSelection(clipPos(cm.doc, pos),
  ...
  function onKeyDown(e) {
    ...
    if (gecko && !mac && !handled && code == 46 && e.shiftKey && !e.ctrlKey
  ...
  function applyTextInput(cm, inserted, deleted, sel, origin) {
    ...
    for (var i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
      ...
      var changeEvent = {
        ...
        origin: origin || (paste ? "paste" : cm.state.cutIncoming > recent
            ? "cut" : "+input")
      };
        ...
        for (var i = 0; i < ranges.length; i++) {
          ...
          if (!range.empty()) {
            ...
            if (from.ch == 0 && ranges.length == newRanges.length
              ...
              replaceOneSelection(this.doc, i,
    ...
    function moveOnce(boundToLine) {
      ...
      if (unit == "codepoint") {
        ...
        if (isNaN(ch)) {
          ...
          next = new Pos(pos.line, Math.max(0, Math.min(lineObj.text.length,
  ...
  ContentEditableInput.prototype.pollSelection = function () {
    ...
    if (android && chrome && this.cm.display.gutterSpecs.length && isInGutter(
        sel.anchorNode)) {
    ...
    options.finishInit = function (cm) {
      ...
      cm.toTextArea = function () {
        ...
        if (textarea.form) {
          ...
          if (!options.leaveSubmitMethodAlone && typeof textarea.form.submit
              == "function") {
