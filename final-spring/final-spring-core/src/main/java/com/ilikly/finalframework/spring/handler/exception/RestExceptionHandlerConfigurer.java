    ...
    public void init() {
        this.exceptionHandlerBeans = BeanUtils.findAllBeansAnnotatedBy(applicationContext, RestExceptionHandler.class)
                .map(it -> {
                    ...
                    if (!(it instanceof com.ilikly.finalframework.spring.handler.exception.ExceptionHandler)) {
                    ...
                    return new ExceptionHandlerBean(order == null ? 0 : order.value(), (com.ilikly.finalframework.spring.handler.exception.ExceptionHandler<Throwable, Object>) it);
                })
                .sorted()
